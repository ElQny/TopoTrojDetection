...
# ---------------------------------------------------------------------
#TODO: replace
#calculates offsets to later crop image to the necessary size (224x224)
def compute_center_crop_offsets_for_224(img_shape: Tuple[int, int, int]) -> Tuple[int, int]:
    """
...
#TODO: replace
#converts the image layout (height, width, channels) -> (channels, height, width)
#this is done for the tensor layout (????) for the DNN
...
#TODO: replace
#image layout (channels, height, width) -> (N, channels, height, width)
#transforms one sample into batch of N=1 (size N=1)
...
#TODO: replace
#normalizes image: shifts all pixel values so minimum= 0
...
#TODO: replace
#normalizes image: intensities are scaled to be between 0 and 1
...
#TODO: replace
#converts the numpy to a pyTorch float tensor
...
#TODO: replace
#turns a raw image into a model input tensor
def img_std(img):
    """
...
#TODO: replace/extend
#refactor since many PointNet-like Models use Conv1d!
...
def extract_interlayer_block(
    ...
    """
    return pd[n_neurons_list[i]:n_neurons_list[i + 1], n_neurons_list[j]:n_neurons_list[j + 1]]
...
def write_symmetric_block_value(maxpool_pd: np.ndarray, i: int, j: int, value: float) -> None:
    """
...
def process_pd(pd: torch.tensor, layer_list: List, sample_n_neurons_list: List = None) -> torch.tensor:
    ...
    """
    n_neurons_list = resolve_neuron_boundaries_for_process_pd(layer_list, sample_n_neurons_list)
...
def resolve_default_second_matrix(X: torch.Tensor, Y: Optional[torch.Tensor]) -> torch.Tensor:
    """
...
def compute_inner_product_gram_from_centered_distances(bpd: torch.Tensor, n: int) -> torch.Tensor:
    """
...
def normalize_by_distance_correlation_denominator(pd: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """
...
def normalize_rows_by_l2_norm(X: torch.Tensor, norms: torch.Tensor, eps: float = 1e-4) -> torch.Tensor:
    """
...
def lower_bound_tensor_with_eps(values: torch.Tensor, eps_tensor: torch.Tensor) -> torch.Tensor:
    """
...
def compute_js_half_entropy_diagonal(paq: torch.Tensor, eps: float = 1e-4) -> torch.Tensor:
    """
...
def compute_js_cross_entropy_term(paq: torch.Tensor, eps: float = 1e-4) -> torch.Tensor:
    """
